...
import { expandPermissionPatterns, permissionKeyMatchesPattern, } from '../../../src/modules/authorization/domain/permission-pattern.js';
const catalog = [
    'tenant.admin',
    'orders.read',
    'orders.create',
...
describe('permission patterns', () => {
    ...
    it('matches exact administrator permissions', () => {
        expect(permissionKeyMatchesPattern('tenant.admin', { kind: 'exact', key: 'tenant.admin' })).toBe(true);
        expect(permissionKeyMatchesPattern('orders.read', { kind: 'exact', key: 'tenant.admin' })).toBe(false);
    });
});
