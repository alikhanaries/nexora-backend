...
describe('idempotency integration', () => {
  ...
  it('replays the same request and rejects key reuse with different body', async () => {
    ...
    const first = await infra.idempotency.execute(
      key,
      fingerprint,
      async () => ({ ok: true }),
    ...
    const replay = await infra.idempotency.execute(
      key,
      fingerprint,
      async () => ({ ok: false }),
      () => ({ statusCode: 200, body: { ok: true } }),
    ...
    await expect(
      infra.idempotency.execute(
        key,
        fingerprintRequest({ value: 2 }),
        async () => ({ ok: true }),
        () => ({ statusCode: 200, body: { ok: true } }),
      ),
  ...
  it('handles concurrent duplicate requests safely', async () => {
    ...
    expect(
      rejected.some((result) => result.reason instanceof IdempotentRequestInProgressError),
    ).toBe(true);
  });
});
