...
function normaliseError(error: unknown): AppError {
  ...
  if (fastifyValidation !== undefined) return fastifyValidation;

  return new InternalError(undefined, error);
}

function readFastifyValidation(error: unknown): AppError | undefined {
  ...
  return new ValidationError('Request validation failed', {
    issues: candidate.validation.map((issue) => {
      ...
      const record = issue as { instancePath?: string; message?: string };
      return {
