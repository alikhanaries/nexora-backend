...
function parseScriptReply(reply, limit) {
...
export class RedisRateLimiter {
    ...
    metrics;
    logger;
    constructor(connection, keys, metrics, logger) {
    ...
    async consume(key) {
        ...
        if (!result.allowed) {
            this.logger.debug({ policy: key.policy.name, retryAfterSeconds: result.retryAfterSeconds }, 'Rate limit exceeded');
        }
        return result;
    }
    async peek(key) {
    ...
    async reset(key) {
        await this.connection.run('ratelimit.reset', () => this.connection.client.del(this.keys.rateLimit(key.policy.name, key.subject)));
    }
    async evaluate(key, cost) {
