import type { CacheService } from '../../shared/cache/index.js';
...
export class RedisCache implements CacheService {
  ...
  async get<T>(namespace: string, key: string): Promise<T | undefined> {
    const raw = await this.connection.run('cache.get', () =>
      this.connection.client.get(this.keys.cache(namespace, key)),
    );
  ...
  async set<T>(namespace: string, key: string, value: T, ttlSeconds: number): Promise<void> {
    ...
    await this.connection.run('cache.set', () =>
      this.connection.client.set(this.keys.cache(namespace, key), serialised, 'EX', ttlSeconds),
    );
  }

  async delete(namespace: string, key: string): Promise<void> {
    await this.connection.run('cache.delete', () =>
      this.connection.client.del(this.keys.cache(namespace, key)),
    );
  }
}
