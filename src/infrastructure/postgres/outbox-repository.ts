...
function toIntegrationEvent(row: z.infer<typeof outboxRowSchema>): IntegrationEvent {
  return {
    id: row.id,
    type: row.event_type,
...
export interface ClaimedEvent extends IntegrationEvent {
  readonly attemptCount: number;
}

export class PostgresOutboxRepository implements EventRecorder {
  constructor(private readonly database: PostgresDatabase) {}

  async record(transaction: Transaction, event: NewIntegrationEvent): Promise<void> {
    await this.recordMany(transaction, [event]);
  }

  async recordMany(
    transaction: Transaction,
    events: readonly NewIntegrationEvent[],
  ): Promise<void> {
   ...
   */
  async claimBatch(batchSize: number, maxAttempts: number): Promise<readonly ClaimedEvent[]> {
    ...
    const result = await this.database.query(
      `WITH claimed AS (
       ...
       RETURNING o.id, o.event_type, o.event_version, o.aggregate_type, o.aggregate_id,
                 o.tenant_id, o.payload, o.correlation_id, o.occurred_at, o.attempt_count`,
      [limit, maxAttempts],
      { operation: 'outbox.claim_batch' },
    );
  ...
  async markPublished(eventIds: readonly string[]): Promise<void> {
    ...
    await this.database.query(
      `UPDATE outbox_events
       ...
       WHERE id = ANY($1::uuid[])`,
      [[...eventIds]],
      { operation: 'outbox.mark_published' },
    );
  }
  ...
  async markFailed(eventId: string, error: string, maxAttempts: number): Promise<void> {
    await this.database.query(
      `UPDATE outbox_events
       ...
       WHERE id = $1`,
      [eventId, error, maxAttempts],
      { operation: 'outbox.mark_failed' },
    );
  }
   ...
   */
  async releaseStaleClaims(staleAfterMs: number): Promise<number> {
    const result = await this.database.query(
      `UPDATE outbox_events
       SET claimed_at = NULL
      ...
      [staleAfterMs / 1_000],
      { operation: 'outbox.release_stale_claims' },
    );
  ...
  async countPending(): Promise<number> {
    const result = await this.database.query(
      ...
      [],
      { operation: 'outbox.count_pending' },
    );
    const parsed = parseOrThrow(
      z.object({ pending: z.number().int() }),
