...
export class OutboxPublisher {
    ...
    running = false;
    stopped = false;
    constructor(outbox, queue, config, logger) {
        this.outbox = outbox;
        this.queue = queue;
        this.config = config;
        this.logger = logger;
    ...
    async tick() {
        ...
        try {
            ...
            for (const event of batch) {
                try {
                    await this.queue.enqueue(QueueName.INTEGRATION_EVENTS, JobName.PUBLISH_INTEGRATION_EVENT, {
                        eventId: event.id,
                    ...
                    this.logger.error({ eventId: event.id, reason: toErrorMessage(error) }, 'Outbox publish failed');
                }
            }
        }
        finally {
