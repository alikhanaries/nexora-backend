...
const MIGRATION_ADVISORY_LOCK_ID = 8147302915n;
...
export async function loadMigrationFiles(directory = MIGRATIONS_DIRECTORY) {
    ...
    for (const filename of sqlFiles) {
        ...
        if (match === null) {
            throw new DatabaseError(`Migration filename "${filename}" must match NNNN_snake_case_name.sql`);
        }
...
function assertChecksumsMatch(applied, available) {
    ...
    for (const record of applied) {
        ...
        if (migration === undefined) {
            throw new DatabaseError(`Migration ${record.version} (${record.name}) is recorded as applied but its file is missing. ` +
                'Refusing to run against a schema this codebase does not describe.');
        }
        if (migration.checksum !== record.checksum) {
            throw new DatabaseError(`Migration ${record.version} (${record.name}) changed after it was applied. ` +
                'Applied migrations are immutable: add a new migration instead.');
        }
    }
}
async function withAdvisoryLock(pool, work) {
...
export async function migrateUp(pool, logger, directory) {
    ...
    return withAdvisoryLock(pool, async (client) => {
        ...
        for (const migration of pending) {
            ...
            try {
                ...
                await client.query('INSERT INTO schema_migrations (version, name, checksum, execution_ms) VALUES ($1, $2, $3, $4)', [migration.version, migration.name, migration.checksum, Date.now() - startedAt]);
                await client.query('COMMIT');
            }
            catch (error) {
                await client.query('ROLLBACK');
                throw new DatabaseError(`Migration ${migration.filename} failed and was rolled back`, error, { version: migration.version, filename: migration.filename });
            }
            executed.push(migration.version);
            logger.info({ version: migration.version, name: migration.name, durationMs: Date.now() - startedAt }, 'Applied migration');
        }
