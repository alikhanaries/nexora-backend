...
import { CustomerSnapshot, type AddressSnapshot } from '../domain/customer-snapshot.js';
...
function normalizeAddress(
  value: z.infer<typeof addressSnapshotSchema> | null,
...
function toOrderLine(row: z.infer<typeof orderLineRowSchema>): OrderLine {
...
function toCustomerSnapshot(row: z.infer<typeof customerRowSchema>): CustomerSnapshot {
...
export class PostgresOrderRepository implements OrderRepository {
  async findById(queryable: Queryable, tenantId: string, orderId: string): Promise<Order | null> {
    const result = await queryable.query(
      `SELECT ${orderSelect} FROM orders WHERE tenant_id = $1 AND id = $2`,
      [tenantId, orderId],
      { operation: 'orders.find_by_id' },
    );
  ...
  async lockOrderForUpdate(
    ...
    const result = await transaction.query(
      `SELECT ${orderSelect}
       ...
       FOR UPDATE`,
      [tenantId, orderId],
      { operation: 'orders.lock_for_update' },
    );
    const row = result.rows[0];
    if (row === undefined) return null;
    return toOrder(parseOrThrow(orderRowSchema, row, 'orders row'));
  ...
  async findByOrderNumber(
    ...
    const result = await queryable.query(
      `SELECT ${orderSelect} FROM orders WHERE tenant_id = $1 AND order_number = $2`,
      [tenantId, orderNumber],
      { operation: 'orders.find_by_order_number' },
    );
    const row = result.rows[0];
    if (row === undefined) return null;
    return toOrder(parseOrThrow(orderRowSchema, row, 'orders row'));
  ...
  async insertOrder(transaction: Transaction, order: Order): Promise<void> {
  ...
  async updateOrder(transaction: Transaction, order: Order): Promise<void> {
    const p = order.toProps();
    await transaction.query(
      `UPDATE orders SET
  ...
  async insertOrderLine(transaction: Transaction, line: OrderLine): Promise<void> {
  ...
  async updateOrderLine(transaction: Transaction, line: OrderLine): Promise<void> {
    const p = line.toProps();
    await transaction.query(
  ...
  async listOrderLines(
    ...
    const result = await queryable.query(
      `SELECT ${orderLineSelect} FROM order_lines WHERE tenant_id = $1 AND order_id = $2 ORDER BY created_at`,
      [tenantId, orderId],
      { operation: 'order_lines.list' },
    );
    return result.rows.map((row) =>
      toOrderLine(parseOrThrow(orderLineRowSchema, row, 'order_lines row')),
    );
  }

  async lockOrderLinesForUpdate(
    transaction: Transaction,
    tenantId: string,
    orderId: string,
  ): Promise<readonly OrderLine[]> {
    const result = await transaction.query(
      `SELECT ${orderLineSelect}
       ...
       FOR UPDATE`,
      [tenantId, orderId],
      { operation: 'order_lines.lock_for_update' },
    );
    return result.rows.map((row) =>
      toOrderLine(parseOrThrow(orderLineRowSchema, row, 'order_lines row')),
    );
  ...
  async lockOrderLineForUpdate(
    ...
    const result = await transaction.query(
      ...
      { operation: 'order_lines.lock_one_for_update' },
    );
    const row = result.rows[0];
    if (row === undefined) return null;
  ...
  async findCustomerSnapshot(
    queryable: Queryable,
    tenantId: string,
    orderId: string,
  ): Promise<CustomerSnapshot | null> {
    const result = await queryable.query(
      `SELECT id, tenant_id, order_id, external_customer_reference, first_name, last_name,
              email, phone, company_name, billing_address, shipping_address, metadata, created_at
       FROM order_customer_snapshots
       WHERE tenant_id = $1 AND order_id = $2`,
      [tenantId, orderId],
      { operation: 'order_customer_snapshots.find' },
    );
    const row = result.rows[0];
    if (row === undefined) return null;
  ...
  async allocateOrderNumber(transaction: Transaction, tenantId: string): Promise<string> {
    const result = await transaction.query(
      `INSERT INTO tenant_order_sequences (tenant_id, last_value)
       VALUES ($1, 1)
      ...
      [tenantId],
      { operation: 'orders.allocate_number' },
    );
    const row = result.rows[0] as { last_value: string | number } | undefined;
    if (row === undefined) {
...
export function orderListCursor(order: Order): string {
