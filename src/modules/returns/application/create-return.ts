...
export interface CreateReturnInput {
  ...
  readonly lines: readonly CreateReturnLineInput[];
...
export interface CreateReturnDependencies {
...
export class CreateReturn {
  constructor(private readonly deps: CreateReturnDependencies) {}

  async execute(input: CreateReturnInput): Promise<CreateReturnResult> {
    ...
    const returnDetail = await this.deps.database.execute(
      async (tx) => {
        ...
        const shipmentId =
          input.shipmentId === undefined || input.shipmentId === null ? null : input.shipmentId;

        if (shipmentId !== null) {
          const valid = await this.deps.returns.verifyShipmentBelongsToOrder(
            tx,
            input.tenantId,
            input.orderId,
            shipmentId,
        ...
        const pendingByLine = await this.deps.returns.sumPendingQuantitiesByOrderLine(
          tx,
          input.tenantId,
          input.orderId,
          orderLineIds,
        ...
        for (const lineInput of input.lines) {
          ...
          const eligibleQuantity =
            orderLine.shippedQuantity - orderLine.returnedQuantity - pendingQuantity;
          ...
          const reason =
            lineInput.reason === undefined || lineInput.reason === null
              ? null
