...
import type {
  ...
  OrderSnapshot,
  ReturnOrderGateway,
} from '../application/ports/order-gateway.js';
...
export class PostgresReturnOrderGateway implements ReturnOrderGateway {
  async findOrder(
    queryable: Queryable,
    tenantId: string,
    orderId: string,
    ...
    const result = await queryable.query(
      ...
      [tenantId, orderId],
      { operation: 'returns.order_gateway.find_order' },
    );
  ...
  async lockOrderLinesForUpdate(
    ...
    const result = await transaction.query(
      `SELECT ${orderLineSelect}
       ...
       FOR UPDATE`,
      [tenantId, orderId],
      { operation: 'returns.order_gateway.lock_order_lines' },
    );
  ...
  async updateOrderLineReturnedQuantity(
    transaction: Transaction,
    tenantId: string,
    orderLineId: string,
    ...
    await transaction.query(
      ...
      [tenantId, orderLineId, returnedQuantity, updatedAt],
      { operation: 'returns.order_gateway.update_returned_quantity' },
    );
  }
}
