...
export interface CreateShipmentInput {
  ...
  readonly lines: readonly CreateShipmentLineInput[];
...
export interface CreateShipmentDependencies {
...
export class CreateShipment {
  constructor(private readonly deps: CreateShipmentDependencies) {}
  ...
  async execute(input: CreateShipmentInput): Promise<CreateShipmentResult> {
    ...
    const shipment = await this.deps.database.execute(
      async (tx) => {
        const lockedLines = await this.deps.orderFulfillmentService.lockOrderLinesForFulfillment(
          input.tenantId,
          input.orderId,
          tx,
        );
        ...
        for (const line of input.lines) {
          ...
          requestedByLine.set(
            line.orderLineId,
            (requestedByLine.get(line.orderLineId) ?? 0) + line.quantity,
          );
        }
        ...
        await this.deps.orderFulfillmentService.applyShipmentQuantities(
          input.tenantId,
          input.orderId,
          [...requestedByLine.entries()].map(([orderLineId, quantity]) => ({
            orderLineId,
            quantity,
          })),
          tx,
