...
import type { PriceListFilters, PriceRepository } from '../domain/price-repository.port.js';
...
function toPrice(row: z.infer<typeof priceRowSchema>): Price {
...
export class PostgresPriceRepository implements PriceRepository {
  async findById(queryable: Queryable, tenantId: string, priceId: string): Promise<Price | null> {
    const result = await queryable.query(
      `SELECT ${priceSelectColumns}
       FROM prices
       WHERE tenant_id = $1 AND id = $2`,
      [tenantId, priceId],
      { operation: 'prices.find_by_id' },
    );
    ...
    const result = await queryable.query(
      `SELECT ${priceSelectColumns}
       ...
       ORDER BY
         ...
         valid_from DESC
       LIMIT 1`,
      [tenantId, productId, currency, at, channelId],
      { operation: 'prices.find_effective' },
    );
  ...
  async insert(transaction: Transaction, price: Price): Promise<void> {
  ...
  async update(transaction: Transaction, price: Price): Promise<void> {
    await transaction.query(
      `UPDATE prices
       SET channel_id = $3,
           amount_minor = $4,
