...
function toBalance(row: z.infer<typeof balanceRowSchema>): InventoryBalance {
...
function toReservation(row: z.infer<typeof reservationRowSchema>): InventoryReservation {
...
export class PostgresInventoryRepository implements InventoryRepository {
  async listBalances(
    ...
    const result = await queryable.query(
      `SELECT id, tenant_id, stock_location_id, product_id, on_hand, reserved, available,
              created_at, updated_at
       FROM inventory_balances
       WHERE ${conditions.join(' AND ')}
       ORDER BY product_id ASC, stock_location_id ASC`,
      params,
      { operation: 'inventory.balances.list' },
    );

    return result.rows.map((row) =>
      toBalance(parseOrThrow(balanceRowSchema, row, 'inventory_balances row')),
    );
  }
  ...
  async ensureBalanceRow(
    ...
    await transaction.query(
      `INSERT INTO inventory_balances
       ...
       ON CONFLICT (tenant_id, stock_location_id, product_id) DO NOTHING`,
      [balanceId, tenantId, stockLocationId, productId],
      { operation: 'inventory.balances.ensure' },
    );
  }
  ...
  async lockBalanceForUpdate(
    ...
    const result = await transaction.query(
      `SELECT id, tenant_id, stock_location_id, product_id, on_hand, reserved, available,
       ...
       FOR UPDATE`,
      [tenantId, stockLocationId, productId],
      { operation: 'inventory.balances.lock_for_update' },
    );
  ...
  async updateBalance(
    ...
    await transaction.query(
      `UPDATE inventory_balances
       ...
       WHERE id = $1`,
      [balanceId, values.onHand, values.reserved, values.available, updatedAt],
      { operation: 'inventory.balances.update' },
    );
  }

  async insertMovement(transaction: Transaction, input: InsertMovementInput): Promise<boolean> {
    const values = [
      input.id,
      input.tenantId,
  ...
  async insertReservation(
    ...
    const existing = await this.findReservationByReference(
      transaction,
      input.tenantId,
      input.referenceType,
      input.referenceId,
  ...
  async findReservationByReference(
    ...
    const result = await transaction.query(
      `SELECT id, tenant_id, stock_location_id, product_id, reference_type, reference_id,
              quantity, status, created_at, released_at
       ...
       WHERE tenant_id = $1
         ...
         AND product_id = $5`,
      [tenantId, referenceType, referenceId, stockLocationId, productId],
      { operation: 'inventory.reservations.find_by_reference' },
    );
  ...
  async markReservationReleased(
    ...
    await transaction.query(
      ...
      [reservationId, releasedAt],
      { operation: 'inventory.reservations.release' },
    );
  }
}
