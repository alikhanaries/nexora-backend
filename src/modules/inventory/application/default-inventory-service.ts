...
import type { AuditEventType } from '../../audit/public/index.js';
...
import type {
  ...
  ReturnInput,
  SaleInput,
} from '../public/inventory-service.js';

export interface DefaultInventoryServiceDeps {
...
export class DefaultInventoryService implements InventoryService {
  constructor(private readonly deps: DefaultInventoryServiceDeps) {}
  ...
  async reserve(input: ReserveInput, tx?: Transaction): Promise<ReserveResult> {
    return this.runInTransaction(input.tenantId, tx, (transaction) =>
      this.reserveInTransaction(transaction, input),
    );
  }

  async release(input: ReleaseInput, tx?: Transaction): Promise<ReleaseResult> {
    return this.runInTransaction(input.tenantId, tx, (transaction) =>
      this.releaseInTransaction(transaction, input),
    );
  }

  async adjust(input: AdjustInput, tx?: Transaction): Promise<InventoryMutationResult> {
    return this.runInTransaction(input.tenantId, tx, (transaction) =>
      this.mutateOnHand(transaction, {
  ...
  async receive(input: ReceiveInput, tx?: Transaction): Promise<InventoryMutationResult> {
    if (input.quantity <= 0) {
  ...
  async recordSale(input: SaleInput, tx?: Transaction): Promise<InventoryMutationResult> {
    if (input.quantity <= 0) {
  ...
  async recordReturn(input: ReturnInput, tx?: Transaction): Promise<InventoryMutationResult> {
    if (input.quantity <= 0) {
  ...
  private async reserveInTransaction(
    ...
    if (!reservationInsert.inserted) {
      ...
      const balance = await this.deps.inventoryRepository.lockBalanceForUpdate(
        transaction,
        input.tenantId,
    ...
    await this.deps.inventoryRepository.updateBalance(
      transaction,
      balance.id,
      { onHand: balance.onHand, reserved, available },
      updatedAt,
    );
  ...
  private async releaseInTransaction(
    ...
    const reservation = await this.deps.inventoryRepository.findReservationByReference(
      transaction,
      input.tenantId,
      input.referenceType,
      input.referenceId,
    ...
    await this.deps.inventoryRepository.markReservationReleased(
      transaction,
      reservation.id,
      updatedAt,
    );
  ...
  private async mutateOnHand(
    ...
    input: {
      ...
      readonly auditEventType: Extract<
        AuditEventType,
        | 'INVENTORY_ADJUSTED'
        | 'INVENTORY_RECEIVED'
    ...
    await this.deps.inventoryRepository.updateBalance(
      transaction,
      balance.id,
      { onHand, reserved: balance.reserved, available },
      updatedAt,
    );
    ...
    if (!inserted && input.idempotencyKey !== undefined) {
      const current = await this.deps.inventoryRepository.lockBalanceForUpdate(
        transaction,
        input.tenantId,
        input.stockLocationId,
        input.productId,
  ...
  private async assertProductExists(
    ...
    const exists = await this.deps.productQueryService.verifyProductBelongsToTenant(
      tenantId,
      productId,
      tx,
    );
  ...
  private async assertActiveStockLocation(
    transaction: Transaction,
    tenantId: string,
    stockLocationId: string,
  ): Promise<void> {
    const location = await this.deps.stockLocationRepository.findById(
      transaction,
      tenantId,
      stockLocationId,
    );
  ...
  private async insertMovementIfNew(
    transaction: Transaction,
    input: {
      readonly tenantId: string;
      readonly stockLocationId: string;
  ...
  private toBalanceSnapshot(balance: {
    readonly onHand: number;
    readonly reserved: number;
    readonly available: number;
  }) {
