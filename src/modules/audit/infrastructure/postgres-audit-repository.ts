...
import type { AuditRepository, ListAuditEventsResult } from '../application/audit-service.js';
...
function mapRow(row: z.infer<typeof auditRowSchema>): AuditEvent {
  return {
    id: row.id,
...
export class PostgresAuditRepository implements AuditRepository {
  async insert(tx: Transaction, event: AuditEventDraft): Promise<void> {
  ...
  async list(
    ...
    const result = await tx.query(
      `SELECT id, tenant_id, actor_kind, actor_id, event_type, resource_type,
