...
import type { CreateRoleRecord, RoleRepository } from '../domain/ports/role-repository.js';
import type { Role, RoleWithPermissions, SystemRoleKey } from '../domain/role.js';
...
export class PostgresRoleRepository implements RoleRepository {
  async create(record: CreateRoleRecord, tx: Transaction): Promise<RoleWithPermissions> {
    const insertResult = await tx.query(
      `INSERT INTO roles (tenant_id, name, system_key, is_system, status)
       VALUES ($1, $2, $3, $4, 'ACTIVE')
       RETURNING id, tenant_id, name, system_key, status, is_system, cloned_from_role_id, created_at, updated_at`,
      [record.tenantId, record.name, record.systemKey, record.isSystem],
      { operation: 'authorization.roles.create' },
    );
  ...
  async findById(roleId: string, tx: Transaction): Promise<Role | null> {
    const result = await tx.query(
      `SELECT id, tenant_id, name, system_key, status, is_system, cloned_from_role_id, created_at, updated_at
       FROM roles
       WHERE id = $1`,
      [roleId],
      { operation: 'authorization.roles.find_by_id' },
    );
  ...
  async findByTenantAndName(tenantId: string, name: string, tx: Transaction): Promise<Role | null> {
    const result = await tx.query(
      `SELECT id, tenant_id, name, system_key, status, is_system, cloned_from_role_id, created_at, updated_at
       FROM roles
       WHERE tenant_id = $1 AND lower(name) = lower($2)`,
      [tenantId, name],
      { operation: 'authorization.roles.find_by_tenant_and_name' },
    );
  ...
  async findSystemRoleByKey(
    ...
    const result = await tx.query(
      ...
      [tenantId, systemKey],
      { operation: 'authorization.roles.find_system_by_key' },
    );
  ...
  async listByTenant(tenantId: string, tx: Transaction): Promise<readonly Role[]> {
    const result = await tx.query(
      `SELECT id, tenant_id, name, system_key, status, is_system, cloned_from_role_id, created_at, updated_at
       FROM roles
       WHERE tenant_id = $1
       ORDER BY name ASC`,
      [tenantId],
      { operation: 'authorization.roles.list_by_tenant' },
    );

    return result.rows.map((row) => mapRole(parseOrThrow(roleRowSchema, row, 'role row')));
  }

  async listPermissionsForRole(roleId: string, tx: Transaction): Promise<readonly Permission[]> {
    const result = await tx.query(
      `SELECT p.id, p.key, p.description, p.created_at
       ...
       ORDER BY p.key ASC`,
      [roleId],
      { operation: 'authorization.roles.list_permissions' },
    );
  ...
  async roleGrantsPermission(
    ...
    const result = await tx.query(
      ...
      { operation: 'authorization.roles.grants_permission' },
    );
  ...
  async setRolePermissions(
    ...
    const permissionResult = await tx.query(
      ...
      [permissionKeys],
      { operation: 'authorization.roles.resolve_permission_ids' },
    );
      ...
      try {
        await tx.query(
          `INSERT INTO role_permissions (role_id, permission_id)
           VALUES ($1, $2)`,
          [roleId, row['id']],
          { operation: 'authorization.roles.add_permission' },
        );
      } catch (error) {
