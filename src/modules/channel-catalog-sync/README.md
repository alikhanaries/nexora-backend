...
Workers register **`nexora-foundation-stub`** (tests / pipeline verification) plus production adapters via `registerMarketplaceCatalogAdapters`. **`shopify`**, **`amazon`**, **`noon`**, and **`namshi`** are fully implemented for verified catalog operations (Phases 23–26). Unregistered keys still fail with `UnsupportedMarketplaceAdapterError` (non-retrying).
