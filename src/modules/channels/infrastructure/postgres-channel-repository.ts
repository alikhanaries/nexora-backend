...
import type { ChannelListFilters, ChannelRepository } from '../domain/channel-repository.port.js';
...
function toChannel(row: z.infer<typeof channelRowSchema>): Channel {
...
export class PostgresChannelRepository implements ChannelRepository {
  async findById(
    ...
    const result = await queryable.query(
      `SELECT id, tenant_id, marketplace_id, name, external_reference, status,
              configuration_reference, created_at, updated_at
       FROM channels
       WHERE tenant_id = $1 AND id = $2`,
      [tenantId, channelId],
      { operation: 'channels.find_by_id' },
    );
  ...
  async insert(transaction: Transaction, channel: Channel): Promise<void> {
  ...
  async update(transaction: Transaction, channel: Channel): Promise<void> {
    await transaction.query(
      `UPDATE channels
       SET name = $3,
